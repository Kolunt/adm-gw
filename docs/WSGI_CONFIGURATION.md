...
Если ошибка сохраняется, используйте версию WSGI с расширенной диагностикой:

1. Скопируйте содержимое файла `backend/wsgi_pythonanywhere.py` в WSGI конфигурацию
2. Этот файл автоматически:
   - Определит username
   - Проверит все возможные пути к проекту
   - Найдет правильный путь к `main.py`
   - Выведет подробную диагностическую информацию

Альтернативно, используйте этот вариант:
