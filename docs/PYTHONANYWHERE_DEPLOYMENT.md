...
2. **Загрузите папку `build` на PythonAnywhere:**
   - Через Files интерфейс загрузите всю папку `build`
   - Или используйте Git и скопируйте после `git pull`
...
3. **Проверьте структуру:**
   ...
   │   ├── main.py
   │   └── ...
   └── build/          # ← Папка с собранным фронтендом
       ├── index.html
       ├── static/
       └── ...
   ```
